...
When a customer message is sent to the `/predict` endpoint, the system predicts the intent and confidence score and stores the result in the database using SQLAlchemy.
...
- Return the stored ticket ID from the prediction API
