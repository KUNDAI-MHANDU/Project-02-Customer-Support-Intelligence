...
The system also exposes a `GET /tickets/{ticket_id}` endpoint to retrieve a stored ticket by its ID.
...
The endpoint returns the stored ticket with the specified ID.
...
```json
{
  "ticket_id": 3,
  "text": "My virtual card keeps getting declined",
  "predicted_intent": "virtual_card_not_working",
  "confidence": 0.7911729096745604,
...
The API currently includes 7 automated tests using `pytest`.
...
The valid prediction test mocks `save_ticket()` so test data is not written to the real PostgreSQL database.

For `GET /tickets/{ticket_id}`:

- Existing ticket → `200`
- Nonexistent ticket → `404`
- Non-integer ticket ID → `422`

The ticket retrieval tests mock `get_ticket_by_id()` so they can test API behavior without querying the real PostgreSQL database.
...
I learned how to use the GET endpoint in FastAPI to return a record from the database using the `get_ticket_by_id()` function and return the record as a JSON response.
